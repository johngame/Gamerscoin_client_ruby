...
require 'uri'
require 'json'
require 'gamerscoin_client'
require 'gamerscoin_client/methods'
...
class GamerscoinClient
  class Client
    ...
    def method_missing(name, *args)
      raise GamerscoinClient::InvalidMethodError.new(name) unless GamerscoinClient::METHODS.include?(name.to_s)
    ...
    def http_post_request(post_body)
      ...
      raise GamerscoinClient::HTTPError.new(response)
    end
    ...
    def get_response_data(http_ok_response)
      ...
      raise GamerscoinClient::RPCError.new(resp['error']['message']) if resp['error'] and http_ok_response.class == Net::HTTPInternalServerError
      resp['result']
    end
    ...
    def get_defaults
      GamerscoinClient.configuration.instance_variables.each.inject({}) {|hash, var|
        hash[var.to_s.delete('@').to_sym] = GamerscoinClient.configuration.instance_variable_get(var);
        hash
      }
    end
